...
class PivotClass(AbstractSheetAdapter):
    ...
    def write_founded_code(self, group: str, code: str, app: Application) -> None:
        tmp_df = pd.DataFrame([{
        ...
        app.create_task(self._write_founded_code_wks(group, code))
    
    async def _write_founded_code_wks(self, group: str, code: str) -> None:
    ...
    def group_found_all_codes_but_last(self, group: str) -> bool:
        if self.valid.empty:
            return False
        founded_codes = self.valid.loc[self.valid['Обозначение группы'] == group]['Код информации'].to_list()
        all_codes_but_last = Info.get_all_codes_but_last()
